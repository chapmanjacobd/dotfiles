...
sudo zypper install -y mpv python3 python3-pip git android-tools R ImageMagick bat code darktable cowsay dnscrypt-proxy exa evince expect fdupes fish gdal fzf geos-devel  gimp git-delta git-lfs htop inotify-tools jq kdiff3 keepassxc kitty krename lato-fonts libreoffice meld moreutils ncdu nmon nomacs picard podman-docker pspg  python ripgrep rstudio-desktop syncthing udunits2 wine xdotool xclip xbacklight wget  postgresql-devel breeze-gtk gdal-devel sqlite-devel bash-completion krusader xmodmap sysstat poppler-tools ghc-postgresql-libpq socat chromium fd
